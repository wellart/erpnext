...
class Company(Document):
	...
	def validate_default_accounts(self):
		for field in ["default_bank_account", "default_cash_account",
			...
			"expenses_included_in_valuation", "default_payroll_payable_account", "default_employee_advance_account", "exchange_gain_loss_account", "disposal_account"]:
				if self.get(field):
	...
	def create_default_warehouses(self):
		for wh_detail in [
			...
			{"warehouse_name": _("Production Warehouse"), "is_group": 1, "parent_warehouse": "All Warehouse"},
			{"warehouse_name": _("Production: Raw Materials"), "is_group": 0, "parent_warehouse": "Production Warehouse"},
			{"warehouse_name": _("Production: Work In Progress"), "is_group": 0, "parent_warehouse": "Production Warehouse"},
			{"warehouse_name": _("Production: Finished Goods"), "is_group": 0, "parent_warehouse": "Production Warehouse"},
			{"warehouse_name": _("Trading Warehouse"), "is_group": 1, "parent_warehouse": "All Warehouse"},
			{"warehouse_name": _("Trading: Stores"), "is_group": 0, "parent_warehouse": "Trading Warehouse"},
			{"warehouse_name": _("Trading: Online Stores"), "is_group": 0, "parent_warehouse": "Trading Warehouse"}]:
...
# default account tambahan
		...
		if not self.series_for_depreciation_entry:
			series_for_depr_entry = "DEPR-"
			self.db_set("series_for_depreciation_entry", series_for_depr_entry)


# end of default account tambahan
	...
	def create_default_cost_center(self):
		cc_list = [
			...
			{
				'cost_center_name':_('Round-Off Cost Center'),
				'company':self.name,
				'is_group':0,
				'parent_cost_center':self.name + ' - ' + self.abbr
			},
			{
				'cost_center_name':_('Depreciation Cost Center'),
				'company':self.name,
				'is_group':0,
				'parent_cost_center':self.name + ' - ' + self.abbr
			},
		...
		frappe.db.set(self, "round_off_cost_center", _("Round-Off Cost Center") + " - " + self.abbr)
		frappe.db.set(self, "depreciation_cost_center", _("Depreciation Cost Center") + " - " + self.abbr)
