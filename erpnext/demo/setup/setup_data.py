...
def complete_setup(domain='Manufacturing'):
	...
	if not frappe.get_all('Company', limit=1):
		setup_complete({
			...
			"email": "pengguna_demo@punia.online",
			"company_tagline": 'Awesome Products and Services',
			"password": "demo",
			"fy_start_date": "2018-01-01",
			"fy_end_date": "2018-12-31",
...
def setup_holiday():
	holiday = frappe.get.doc({
		"doctype": "Holiday",
		"holiday_date": "{0}-01-01".format(year),
	})
	holiday.insert()
	holiday.holiday_date = "2018-04-22"
	holiday.description = "Libur Bersama"
	holiday.save()
